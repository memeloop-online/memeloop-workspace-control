...
pub(super) async fn insert_placement_updated_sqlite(
    connection: &mut SqliteConnection,
    installation_id: &str,
    workspace: &Workspace,
    now: i64,
    ...
    let event_id = Uuid::now_v7();
    sqlx::query("INSERT INTO events (id, installation_id, organization_id, workspace_id, kind, payload_json, created_at) VALUES (?1, ?2, ?3, ?4, 'workspace.placement_updated', ?5, ?6)")
        .bind(event_id.to_string())
        .bind(installation_id)
        .bind(workspace.organization_id.to_string())
        .bind(workspace.id.to_string())
        .bind(placement_updated_payload(workspace).to_string())
        .bind(now)
        .execute(&mut *connection)
        .await?;
    super::webhook_store::enqueue_sqlite(
        ...
        workspace.id,
        event_id,
        "workspace.placement_updated",
        now,
    )
    .await
}

pub(super) async fn insert_placement_updated_postgres(
    connection: &mut PgConnection,
    installation_id: &str,
    workspace: &Workspace,
    now: i64,
...
fn placement_updated_payload(workspace: &Workspace) -> serde_json::Value {
    serde_json::json!({
        "node_pool": workspace.node_pool,
        "generation": workspace.generation,
    })
