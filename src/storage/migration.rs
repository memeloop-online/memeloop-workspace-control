...
async fn migrate_sqlite(pool: &SqlitePool, applied_at: i64) -> Result<(), StorageError> {
    ...
    for (_, statements) in COMMON_MIGRATION_GROUPS
        .iter()
        .chain([(17, schema::V17_SQLITE_MIGRATIONS)].iter())
...
async fn migrate_postgres(
    ...
    for (_, statements) in COMMON_MIGRATION_GROUPS
        .iter()
        .chain([(17, schema::V17_POSTGRES_MIGRATIONS)].iter())
        .filter(|(introduced_in, _)| version < *introduced_in)
    {
        for statement in *statements {
            sqlx::query(statement).execute(&mut *transaction).await?;
...
mod tests {
    ...
    async fn sqlite_membership_organization_role_index_has_expected_columns() {
        ...
        assert_eq!(database.schema_version().await.unwrap(), 17);
    ...
    async fn sqlite_v17_removes_wildcard_and_unbounded_api_keys() {
        ...
        sqlx::query("DELETE FROM schema_migrations WHERE version = 17")
            ...
            .await
            .unwrap();
        sqlx::query("INSERT INTO schema_migrations (version, applied_at) VALUES (16, ?1)")
            .bind(now)
            .execute(pool)
            .await
            .unwrap();
