...
for namespace in "$TARGET_NAMESPACE" "$CONTROL_NAMESPACE" "${WORKSPACE_NAMESPACES[@]}"; do
...
section "Control plane and workspace PVC -> PV bindings"
...
mapfile -t PVS < <(kubectl get pvc -A -o jsonpath='{range .items[?(@.spec.volumeName)]}{.metadata.namespace}{" "}{.spec.volumeName}{"\n"}{end}' \
  | awk -v c="$CONTROL_NAMESPACE" -v a="${WORKSPACE_NAMESPACES[0]}" -v b="${WORKSPACE_NAMESPACES[1]}" -v d="${WORKSPACE_NAMESPACES[2]}" -v e="${WORKSPACE_NAMESPACES[3]}" '$1==c || $1==a || $1==b || $1==d || $1==e {print $2}')
for pv in "${PVS[@]}"; do
...
section "Workload ownership and readiness"
...
echo 'Read-only report complete. Treat any missing, degraded, faulted, attached-to-unexpected-node, or claim-UID mismatch as a stop gate.'
