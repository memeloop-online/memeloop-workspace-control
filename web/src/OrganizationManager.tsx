...
export function OrganizationManager({
  ...
  return <div className="system-card">
    ...
    {organization ? <>
      ...
      <div className="form-actions"><button className="button" disabled={!organizationName.trim() || organizationName.trim() === organization.name} onClick={onSave}>{t("saveOrganization")}</button>{canDelete && <button className="button danger" onClick={onDelete}>{t("deleteOrganization")}</button>}</div>
