...
/** View model for a single credential row; filtering happens in the caller. */
export interface CredentialListItem {
  ...
  sensitive: boolean;
  locked: boolean;
  /** Translated kind label shown in the icon tooltip. */
  kindLabel: string;
}

export interface CredentialListLabels {
  title: string;
  emptyLabel: string;
...
export interface CredentialListProps {
  /** Already-filtered items to render; the count badge reflects this list. */
  items: CredentialListItem[];
  selectedKey: string | null;
  search: string;
  loading?: boolean;
  labels: CredentialListLabels;
  onSearchChange: (value: string) => void;
  onSelect: (item: CredentialListItem) => void;
}
...
/** Searchable, selectable credential list with the injection panel's visual language. */
  ...
  return (
    <Card className={styles.card} appearance="outline">
      <CardHeader
        ...
        header={<Text weight="semibold">{labels.title}</Text>}
        description={
      ...
      <div className={styles.results} aria-busy={loading}>
        {loading && <Text className={styles.empty}>{labels.loading}</Text>}
        {!loading && items.length === 0 && <Text className={styles.empty}>{search.trim() ? labels.noFilteredResults : labels.emptyLabel}</Text>}
        {!loading && items.map((item) => {
          const selected = selectedKey === item.key;
          const kindDescription = `${item.kindLabel} · ${item.sensitive || item.kind === "secret_file" ? labels.sensitiveValue : labels.visibleConfiguration}`;
          return (
            <Button
              key={item.key}
              type="button"
            ...
            >
              ...
              <Badge className={styles.version} appearance="tint" color={item.locked ? "warning" : "informative"}>
                v{item.version}{item.locked ? ` · ${labels.lockedState}` : ""}
              </Badge>
            </Button>
          );
        })}
...
function kindIcon(item: CredentialListItem) {
...
function kindIconStyle(item: CredentialListItem, styles: Record<"iconFile" | "iconVariable" | "iconKey" | "iconSensitive", string>) {
