...
import type { ImagePolicy, Principal, Resources, Role, ScalingStatus, UserSummary, WebhookSubscription, WorkspaceResponse, WorkspaceTemplate } from "./types";
...
export function AdminPanel({ api, principal, organizationId, workspaces, onError }: { api: ApiClient; principal: Principal; organizationId: string; workspaces: WorkspaceResponse[]; onError: (message: string) => void }) {
  const { t } = useI18n();
  ...
  async function refresh() {
    try {
      const common = [api.quota(organizationId), api.templates(organizationId), api.webhooks(organizationId)] as const;
      const [currentQuota, visibleTemplates, subscriptions] = await Promise.all(common);
      setQuota(currentQuota); setQuotaDraft(resourceDraft(currentQuota ?? DEFAULT_QUOTA)); setTemplates(visibleTemplates); setWebhooks(subscriptions);
  ...
  return <section className="panel-stack">
    <div className="section-heading"><div><p className="eyebrow">ADMINISTRATION</p><h2>{t("administrationTitle")}</h2></div><a className="button" href="/api/v1/openapi.json" target="_blank">OpenAPI</a></div>
