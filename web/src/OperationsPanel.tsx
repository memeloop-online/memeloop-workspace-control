...
const DEFAULT_QUOTA: QuotaResources = { cpu_millis: 4000, memory_mib: 8192, gpu_count: 0, disk_gib: 100 };
...
function IdentityQuotaCard({ principal, quota, canManageQuota, editingQuota, quotaDraft, onEdit, onCancel, onSave, onDraftChange }: { principal: Principal; quota: QuotaResources | null; canManageQuota: boolean; editingQuota: boolean; quotaDraft: ResourceDraft; onEdit: () => void; onCancel: () => void; onSave: () => void; onDraftChange: (draft: ResourceDraft) => void }) {
  const { t } = useI18n();
  const styles = useAdminStyles();
  return <AdminCard title={t("identityQuota")}><dl className={styles.definitionList}><div className={styles.definitionItem}><dt className={styles.definitionTerm}>{t("user")}</dt><dd className={styles.definitionValue}>{principal.display_name}</dd></div><div className={styles.definitionItem}><dt className={styles.definitionTerm}>{t("systemAdmin")}</dt><dd className={styles.definitionValue}>{principal.system_admin ? t("enabled") : t("disabled")}</dd></div><div className={styles.definitionItem}><dt className={styles.definitionTerm}>{t("orgQuota")}</dt><dd className={styles.definitionValue}>{quota ? `${quota.cpu_millis}m / ${quota.memory_mib}Mi / ${quota.disk_gib}Gi / ${quota.gpu_count} ${t("gpu")}` : t("notEnabled")}</dd></div></dl>{editingQuota ? <div className={styles.stack}><div className={styles.formGrid}><ResourceInput label={`${t("cpu")} (m)`} value={quotaDraft.cpu_millis} min={100} step={100} onChange={(cpu_millis) => onDraftChange({ ...quotaDraft, cpu_millis })} /><ResourceInput label={`${t("memory")} (MiB)`} value={quotaDraft.memory_mib} min={128} step={128} onChange={(memory_mib) => onDraftChange({ ...quotaDraft, memory_mib })} /><ResourceInput label={t("gpu")} value={quotaDraft.gpu_count} min={0} step={1} onChange={(gpu_count) => onDraftChange({ ...quotaDraft, gpu_count })} /><ResourceInput label={`${t("disk")} (GiB)`} value={quotaDraft.disk_gib} min={1} step={1} onChange={(disk_gib) => onDraftChange({ ...quotaDraft, disk_gib })} /></div><div className={styles.actions}><SaveButton onClick={onSave}>{t("saveQuota")}</SaveButton><Button appearance="secondary" onClick={onCancel}>{t("cancel")}</Button></div></div> : <Button disabled={!canManageQuota} title={canManageQuota ? undefined : t("noQuotaPermission")} onClick={onEdit}>{t("editQuota")}</Button>}</AdminCard>;
}
...
function resourceDraft(resources: QuotaResources): ResourceDraft {
  return { cpu_millis: String(resources.cpu_millis), memory_mib: String(resources.memory_mib), gpu_count: String(resources.gpu_count), disk_gib: String(resources.disk_gib) };
}

function parseResourceDraft(draft: ResourceDraft): QuotaResources {
  return { cpu_millis: parseResourceValue(draft.cpu_millis, 100, 100), memory_mib: parseResourceValue(draft.memory_mib, 128, 128), gpu_count: parseResourceValue(draft.gpu_count, 0, 1), disk_gib: parseResourceValue(draft.disk_gib, 1, 1) };
}
