...
| UI-02 | Total actual usage in workspace summary | In implementation: organization usage-summary API (bounded Prometheus aggregation, template-key authorization, nullable telemetry) and matching frontend in parallel. Existing Prometheus organization labels verified. Totals must ignore search/page; no current-page sums. CI/deployment pending. |
