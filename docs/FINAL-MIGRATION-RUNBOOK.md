...
`k3si-7032544955` retained and every workspace resource name qualified by its workspace ID. Do
not start this procedure from the currently running Coder TOKEN center dev workspace: an external
agent must perform its cutover because its source claim is 100 GiB and is in scope for preservation.
...
| rust-dev-test / Coder TOKEN center dev | `ws-k3si-7032544955-b405d2441a1ee149` / `workspace-data-w-b405d2441a1ee149-0` (100Gi) | `pvc-fe7130d7-18a2-470e-824e-a0dea84e9a5e`, Longhorn, `Delete` | **External agent only. Blocked:** PV `claimRef.uid` was `60f341a2-37fb-4d76-80c4-bc657ab0a44a`; compare it to the live PVC UID and explain any mismatch before touching it. |

All Longhorn entries must be `healthy`, attached only where expected, and have no outstanding
replica rebuild/fault condition at the freeze point. `Delete` is unsafe during rebinding: set each
selected PV reclaim policy to `Retain` and verify it before its old PVC is removed. Record all PV,
PVC, VolumeAttachment, Longhorn Volume, snapshot, node, claim UID and manifest digests in the
external change record; do not record secret values or database contents.
...
3. Promote schema **19 -> 20 only** using the published 19-to-20 bridge release. Read the
   ...
   next release.
4. Promote the published **20 -> 21 bridge** after the schema-20 acceptance evidence is complete.
   Do not jump 19 directly to 21, and do not use the in-development schema-22 key-column worker
   release in production. Schema compatibility is a hard gate, not a cosmetic version bump.
5. Freeze writes and stop every workspace from an external operator context, including the active
   100Gi Coder workspace. Confirm Pods are gone, StatefulSets cannot recreate them, no
...
6. Migrate the control-plane database and workload manifests into the target namespace, then move
   the four workspace PV bindings one at a time. Start a single writer only after its binding and
   integrity checks pass. Do not simultaneously mount old and target claims.
7. Validate each workspace (PVC/PV identity, SSH host-key continuity, authenticated SSH command,
...
This procedure applies only to the four listed Longhorn PVs after all gates pass. It is a human
runbook so each irreversible action receives a separate review.

1. Capture `kubectl get pvc,pv,volumeattachment` metadata and Longhorn volume state. Require a
   matching old PVC UID in `PV.spec.claimRef.uid`; the 100Gi mismatch is a stop condition.
...
5. Verify Longhorn reports precisely the intended PV and, before starting the StatefulSet, that it
   is detached and healthy. Start exactly one target workload; confirm its attachment node and
   writer. Restore `Retain` only after the agreed rollback policy says the old binding can no
   longer be recovered.
...
Confirm schema version, installation ID, namespace configuration, all four workspace records,
...
four workspace identities; all PV claimRefs point to target claims with matching UIDs; each
...
Current source namespaces are mwc-k3si-7032544955 and ws-k3si-7032544955-{bd2dc9ca6aa2b1b5,
b268ff46894a14b9,97645a0fb4771b1d,b405d2441a1ee149}. The 100Gi PVC is
workspace-data-w-b405d2441a1ee149-0; it needs an external stop and has a recorded PV claimRef UID
mismatch that must be explained before action. game-forking's Longhorn volume was degraded and
must be healthy first. Do not expose secrets or database contents.

Pause Argo CD and every old reconciler before moving claims. Take and verify fresh Longhorn
snapshots. Release production schema 19->20 using the published bridge, validate it, then release
20->21 using the published bridge; read exact image digests from successful CI provenance rather
than guessing. Schema 22 is development-only and prohibited. Stop all writers, confirm detachment,
...
Validate PV/PVC identity, Longhorn attachments, schema, SSH host keys, SSH, Web Shell, routes,
and durable data for all four workspaces. Keep old namespaces/controllers/PVs for the agreed
